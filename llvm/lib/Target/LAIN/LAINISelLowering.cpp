...
#include "LAINGenCallingConv.inc"
...
SDValue LAINTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_LAIN);
  ...
  Chain = DAG.getNode(LAINISD::CALL, DL, NodeTys, Ops);
  ...
  RetCCInfo.AnalyzeCallResult(Ins, RetCC_LAIN);
...
/// LAIN formal arguments implementation
// TODO: rewrite
SDValue LAINTargetLowering::LowerFormalArguments(
  ...
  SmallVector<CCValAssign, 16> ArgLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, ArgLocs, *DAG.getContext());
  CCInfo.AnalyzeFormalArguments(Ins, CC_LAIN);
  ...
  if (IsVarArg) {
    ...
    const TargetRegisterClass *RC = &LAIN::GPRRegClass;
    MachineFrameInfo &MFI = MF.getFrameInfo();
    MachineRegisterInfo &RegInfo = MF.getRegInfo();
    LAINFunctionInfo *UFI = MF.getInfo<LAINFunctionInfo>();
...
bool LAINTargetLowering::CanLowerReturn(
    CallingConv::ID CallConv, MachineFunction &MF, bool IsVarArg,
    const SmallVectorImpl<ISD::OutputArg> &Outs, LLVMContext &Context) const {
  SmallVector<CCValAssign, 16> RVLocs;
  CCState CCInfo(CallConv, IsVarArg, MF, RVLocs, Context);
  if (!CCInfo.CheckReturn(Outs, RetCC_LAIN))
    return false;
...
LAINTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_LAIN);
  ...
  return DAG.getNode(LAINISD::RET, DL, MVT::Other, RetOps);
}
