...
void LAINFrameLowering::determineCalleeSaves(MachineFunction &MF,
  ...
  // pointer.
  if (hasFP(MF)) {
    SavedRegs.set(LAIN::RA);
    SavedRegs.set(LAIN::FP);
  }
  // Mark BP as used if function has dedicated base pointer.
  if (hasBP(MF))
    SavedRegs.set(LAIN::BP);
}

// TODO: Build insns
void LAINFrameLowering::adjustReg(MachineBasicBlock &MBB,
                                  MachineBasicBlock::iterator MBBI,
                                  const DebugLoc &DL, Register DestReg,
                                  Register SrcReg, int64_t Val,
                                  MachineInstr::MIFlag Flag) const {
  // MachineRegisterInfo &MRI = MBB.getParent()->getRegInfo();
  const LAINInstrInfo *TII = STI.getInstrInfo();

  if (DestReg == SrcReg && Val == 0)
    return;

  if (isInt<16>(Val)) {
    BuildMI(MBB, MBBI, DL, TII->get(LAIN::ADDI), DestReg)
        .addReg(SrcReg)
        .addImm(Val)
        .setMIFlag(Flag);
  } else {
...
void LAINFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  auto *FI = MF.getInfo<LAINFunctionInfo>();
  const LAINRegisterInfo *RI = STI.getRegisterInfo();
  MachineBasicBlock::iterator MBBI = MBB.begin();

  Register FPReg = LAIN::FP;
  Register SPReg = LAIN::SP;
  ...
  if (!isInt<16>(StackSize)) {
    llvm_unreachable("Stack offs won't fit in LAIN::LDi");
  }
...
void LAINFrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  const LAINRegisterInfo *RI = STI.getRegisterInfo();
  MachineFrameInfo &MFI = MF.getFrameInfo();
  auto *UFI = MF.getInfo<LAINFunctionInfo>();
  Register FPReg = LAIN::FP;
  Register SPReg = LAIN::SP;
...
void LAINFrameLowering::processFunctionBeforeFrameFinalized(
...
// TODO: seems ok
MachineBasicBlock::iterator LAINFrameLowering::eliminateCallFramePseudoInstr(
  ...
  if (!hasReservedCallFrame(MF)) {
    ...
    if (Amount != 0) {
      ...
      if (MI->getOpcode() == LAIN::ADJCALLSTACKDOWN)
        Amount = -Amount;
...
bool LAINFrameLowering::hasBP(const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
