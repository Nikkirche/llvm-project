...
class LAINAsmPrinter : public AsmPrinter {
  const MCSubtargetInfo *STI;

public:
  explicit LAINAsmPrinter(TargetMachine &TM,
                         std::unique_ptr<MCStreamer> Streamer)
      : AsmPrinter(TM, std::move(Streamer)), STI(TM.getMCSubtargetInfo()) {}

  StringRef getPassName() const override { return "LAIN Assembly Printer"; }
...
void LAINAsmPrinter::emitInstruction(const MachineInstr *MI) {
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializeLAINAsmPrinter() {
  RegisterAsmPrinter<LAINAsmPrinter> X(getTheLAINTarget());
}